  ...
  sketch.draw = function() {
    ...
    for (let k = 0; k < 6; k++) { // k loop all characters of the text
      for (let i = 6; i >= 0; i--) { // i loop all 7 lines of a character in reverse
        for (let j = 0; j < 6; j++) { // j loop all 5 columns of a character
            ...
            radial_step*(angle + k*7 + j), // starting at the current rotation + character position k + column j of the caracter
            radial_step*(angle + k*7 + j+1)); // ending +1 further
            
            //...and right arc
            sketch.arc(sketch.width, sketch.height/2, diameter_step*(i+3), diameter_step*(i+3), radial_step*(angle+k*7+j), radial_step*(angle+k*7+j+1));
        }
      }
    }
    
    // make a small black circle for the interior
    sketch.fill(0);
    sketch.ellipse(0, sketch.height/2, diameter_step*2);
    sketch.ellipse(sketch.width,  sketch.height/2, diameter_step*2);
  }
}
